...
$\frac{dy}{dx} = \frac{df(x)}{dx} = \frac{d u_2(x,u_1)}{d x} = \frac{\partial u_2}{\partial x}\frac{dx}{dx} + \frac{\partial u_2}{\partial u_1}\frac{d u_1}{d x} = \frac{\partial u_2}{\partial x} + \frac{\partial u_2}{\partial u_1}\frac{d u_1}{d x}$

Using this formula, we get the proper answer:

$\frac{dy}{dx} = \frac{\partial u_2}{\partial x} + \frac{\partial u_2}{\partial u_1}\frac{d u_1}{d x} = 1 + 1 \times 2x = 1 + 2x$

That is an application of what we call the {\em single-variable total-derivative chain rule}:

\[
\frac{d f(x,u_1,\ldots,u_n)}{d x} = \frac{\partial f}{\partial x} + \frac{\partial f}{\partial u_1}\frac{d u_1}{d x} + \frac{\partial f}{\partial u_2}\frac{d u_2}{d x} + \ldots + \frac{\partial f}{\partial u_n}\frac{d u_n}{dx} = \frac{\partial f}{\partial x} + \sum_{i=1}^n \frac{\partial f}{\partial u_i}\frac{d u_i}{d x}
\]

There is something subtle going on here with the notation because we are mixing derivative and partial derivative notation. Although $f(x,u_1,\ldots,u_n)$ is a function of multiple parameters, we are deliberately using notation ${d f(x,u_1,\ldots,u_n)}/{d x}$ rather than ${\partial f(x,u_1,\ldots,u_n)}/{\partial x}$ to emphasize the fact we're computing the total derivative. Similarly, we use ${d u_i}/{d x}$ not ${\partial u_i}/{\partial x}$ for the intermediate variables to indicate total derivative not partial derivative. The total derivative assumes all variables are potentially codependent whereas the partial derivative assumes all variables but $x$ are constants. This notation mirrors that of [Wikipedia](https://en.wikipedia.org/wiki/Total\_derivative) but differs from [Wolfram's notation](http://mathworld.wolfram.com/TotalDerivative.html). Wolfram uses $\frac{\partial f(x,u_1,\ldots,u_n)}{\partial x}$, which is technically more correct but confusing because the partial and total derivatives are different.

Also notice that the total derivative formula always adds terms $\frac{\partial f}{\partial u_i}\frac{d u_i}{d x}$.  It's tempting to think that adding terms in the derivative makes sense because, for example, $y = x+x^2$ adds two terms. Nope. The total derivative is adding terms because it represents a weighted sum of all $x$ contributions to the change in $y$. For example, if we change the addition to multiplication in $y = x \times x^2$, the derivative formula still adds partial derivative terms. ($x \times x^2$  simplifies to $x^3$ but for this demonstration, let's not combine the terms.) Here are the intermediate variables and partial derivatives:
...
$\frac{dy}{dx} = \frac{\partial u_2}{\partial x} + \frac{\partial u_2}{\partial u_1}\frac{d u_1}{d x} = u_1 + x \times 2x = x^2 + 2x^2 = 3x^2$
